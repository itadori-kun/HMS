...
app.route('/').get(async(req,res)=>{
  const medication = await medicationModel.find().populate({path:'card_no', select:['card_no']}).exec()
...
.put(async(req, res)=>{
  ...
  try {
    const medication = await medicationModel.findOneAndUpdate({_id:req.params.id}, req.body, {new:true})
  res.json({
    code:200,
...
.get(async(req,res)=>{
  ...
  try {
    const medication = await medicationModel.findOne({_id:req.params.id})
...
.delete(async(req,res)=>{
  ...
  try {
    const medication = await medicationModel.findOne({_id:req.params.id}).exec()
